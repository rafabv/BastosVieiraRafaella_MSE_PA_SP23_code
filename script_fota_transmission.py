...
def read_file():
    file = ns2fwu.HexFile('fota_toggleled_1s.hex')
    bin_file = ns2fwu.BinaryFile(file)
...
async def uart_terminal():
    ...
    async with BleakClient(device, disconnected_callback=handle_disconnect) as client:
        ...
        sliced_data = list(sliced(data, 60))
        sliced_data_header  = []
        head_start = [0x80]
        head_stand = [0x00]
        
        for k in range (0,len(sliced_data)):
            temp = bytearray()
            temp = sliced_data[k]
            if k <= 0 :
                temp[0:0] = head_start
                ...
                temp[0:0] = head_stand
            sliced_data_header.append(temp)
   
        for s in sliced_data_header:
            print([f"{v:02X}" for v in s])
        ...
        await client.write_gatt_char(rx_char, endoftransmission)
